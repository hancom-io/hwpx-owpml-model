...
namespace OWPML {
    ...
    CPolygonType* CPolygonType::Create()
    {
        return new CPolygonType();
    }

    CPolygonType* CPolygonType::Clone()
    {
        CPolygonType* clone = new CPolygonType();

        clone->SetObjectList(this->GetObjectList());
        return clone;
    }

    CPolygonType::CPolygonType() : CAbstractDrawingObjectType(ID_PARA_PolygonType)
    {
    }

    CPolygonType::~CPolygonType()
    {
    }

    void CPolygonType::InitMap(bool bRead)
    {
        ...
        OWPML_PARALIST_REDIRECT_FUNC(CAbstractShapeObjectType, sz)
            ...
            OWPML_CORE_REDIRECT_FUNC(CPolygonType, pt)

            OWPML_BEGIN_IMPLEMENT_FUNCMAP(CPolygonType)
            // "AbstractShapeObjectType.h"
    ...
    bool CPolygonType::WriteElement(LPCWSTR pCurObjName, CSerializer* serialize, CAttribute* att)
    {
    ...
    bool CPolygonType::ReadAttribute(CAttribute* att)
    {
        bool bres = TRUE;
    ...
    CPointType* CPolygonType::Setpt(CPointType* ppt)
    {
        CPointType* pt = ppt;
        if (pt == NULL) {
    ...
    CPointType* CPolygonType::Getpt(int index)
    {
