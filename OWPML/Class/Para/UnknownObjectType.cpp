...
#include "UnknownObjectType.h"

namespace OWPML {

    CUnknownObjectType* CUnknownObjectType::Create()
    {
        return new CUnknownObjectType();
    }

    CUnknownObjectType::CUnknownObjectType() : CAbstractDrawingObjectType(ID_PARA_UnknownObject_ID), m_native_ctrlid(0), m_bUnidentified(false)
    {
    }

    CUnknownObjectType::~CUnknownObjectType()
    {
    }

    void CUnknownObjectType::InitMap(bool bRead)
    {
        ...
        OWPML_PARALIST_REDIRECT_FUNC(CAbstractShapeObjectType, sz)
            ...
            OWPML_CORE_REDIRECT_FUNC(CUnknownObjectType, pt0)
            OWPML_CORE_REDIRECT_FUNC(CUnknownObjectType, pt1)
            OWPML_CORE_REDIRECT_FUNC(CUnknownObjectType, pt2)
            OWPML_CORE_REDIRECT_FUNC(CUnknownObjectType, pt3)

            OWPML_BEGIN_IMPLEMENT_FUNCMAP(CUnknownObjectType)
            // "AbstractShapeObjectType.h"
    ...
    bool CUnknownObjectType::WriteElement(LPCWSTR pCurObjName, CSerializer* serialize, CAttribute* att)
    {
        CAbstractDrawingObjectType::WriteElement(pCurObjName, serialize, att);
        Util::CXMLAttConverter::SetAttribute(att, L"nativectrlid", m_native_ctrlid);

        return serialize->WriteElement(pCurObjName, this, att, NULL);
    }

    bool CUnknownObjectType::ReadAttribute(CAttribute* att)
    {
        bool bres = TRUE;
        bres &= CAbstractDrawingObjectType::ReadAttribute(att);

        if (!Util::CXMLAttConverter::GetAttribute(att, L"nativectrlid", m_native_ctrlid)) {
    ...
    CPointType* CUnknownObjectType::Setpt0(CPointType* ppt0)
    {
        CPointType* pt0 = ppt0;
        if (pt0 == NULL) {
    ...
    CPointType* CUnknownObjectType::Getpt0(int index)
    {
        return static_cast<CPointType*>(GetObjectByID(ID_CORE_PointType_Pt0, index));
    }

    CPointType* CUnknownObjectType::Setpt1(CPointType* ppt1)
    {
        CPointType* pt1 = ppt1;
        if (pt1 == NULL) {
    ...
    CPointType* CUnknownObjectType::Getpt1(int index)
    {
        return static_cast<CPointType*>(GetObjectByID(ID_CORE_PointType_Pt1, index));
    }

    CPointType* CUnknownObjectType::Setpt2(CPointType* ppt2)
    {
        CPointType* pt2 = ppt2;
        if (pt2 == NULL) {
    ...
    CPointType* CUnknownObjectType::Getpt2(int index)
    {
        return static_cast<CPointType*>(GetObjectByID(ID_CORE_PointType_Pt2, index));
    }

    CPointType* CUnknownObjectType::Setpt3(CPointType* ppt3)
    {
        CPointType* pt3 = ppt3;
        if (pt3 == NULL) {
    ...
    CPointType* CUnknownObjectType::Getpt3(int index)
    {
